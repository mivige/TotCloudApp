...
-- DS_DATACENTERREGION table backup
...
END$$
DELIMITER ;
TABLA DS_STORAGE_BACKUP_INCREMENTAL
...
END$$
DELIMITER ;
TABLA PAAS_DEDICATED_SERVER_BACKUP_INCREMENTAL
...
En MySQL, los triggers no se activan automáticamente por acciones realizadas a través de la cláusula ON 
DELETE CASCADE, ya que esta acción ocurre internamente dentro del motor de base de datos y no dispara 
eventos explícitos como DELETE.
DELIMITER $$
CREATE TRIGGER request_before_delete
BEFORE DELETE ON request
FOR EACH ROW
BEGIN
 -- Copia los registros relacionados de paas_dedicated_server a la tabla de respaldo antes de que sean 
eliminados
 INSERT INTO paas_dedicated_server_backup_incremental
 SELECT 
...
END$$
DELIMITER ;
TABLA REQUEST_BACKUP_INCREMENTAL
...
END$$
DELIMITER ;
TABLA U_PASSWORD_HISTORY_BACKUP_INCREMENTAL
...
END$$
DELIMITER ;
TABLA U_ROLE_BACKUP_INCREMENTAL
...
Este trigger se ha añadido debido al on cascade de la tabla u_role con 
u_user_role_backup que Mariadb no lanza los triggers
CREATE TRIGGER u_role_before_delete
...
END$$
DELIMITER ;
TABLA U_USER_X_ROLE_BACKUP_INCREMENTAL
...
END$$
DELIMITER ;
EVENT DAILY_INCREMENTAL_BACKUP
Es el evento diario que se encarga de ejecutar todos los procedimientos de exportación a ficheros 
de las tablas de respaldo. De este modo, se consigue realizar un backup diario incremental de 
