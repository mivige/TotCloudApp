...
$resultado="ERROR";   
   if($modificar==1){
    $stmt = $dbb->prepare('update ds_public_bandwidth  set price=?,description=?,code=? where  id=?  ');
	$dbb->set_charset("utf8");
	$stmt->bind_param('dssd',$price,$description,$code,$id);
   }else{
    $stmt = $dbb->prepare('insert into ds_public_bandwidth (price,description,code,currency_type) values (?,?,?,"978") ');
	$dbb->set_charset("utf8");
