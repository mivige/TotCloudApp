...
$resultado="ERROR";   
   if($modificar==1){
    ...
    $stmt = $dbb->prepare('insert into ds_datacenterregion (price,description,code) values (?,?,?) ');
	$dbb->set_charset("utf8");
	$stmt->bind_param('dss',$price,$description,$code);

   }
	if ($stmt->execute()){
		header("Location: ../../index.php?opcion=paas_datacenterregion&error=2");
        exit; 
	} else 
	{
		header("Location: ../../index.php?opcion=paas_datacenterregion&error=1");
	$resultado = 'KO'; 	
	exit; 
	}
	echo $resultado;
