...
$resultado="ERROR";   
   if($modificar==1){
    $stmt = $dbb->prepare('update ds_processor  set price=?,description=?,code=? where  id=?  ');
	$dbb->set_charset("utf8");
	$stmt->bind_param('dssd',$price,$description,$code,$id);
   }else{
    $stmt = $dbb->prepare('insert into ds_processor (price,description,code,currency_type) values (?,?,?,"978") ');
	$dbb->set_charset("utf8");
